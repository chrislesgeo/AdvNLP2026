...
class DatasetSummary(Dataset):
    ...
    def handleentfile(self, entpath):
        if not os.path.isfile(entpath):
            print(f"Warning: missing entity file {entpath}. Proceeding without entity guidance.")
            return {}
        fe = open(entpath, 'r', encoding='utf-8')
        allres = {}
        while True:
    ...
    def getalldata(self, filename):
        # Dataset files may contain characters outside cp1252 on Windows.
