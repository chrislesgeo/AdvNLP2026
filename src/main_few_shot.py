...
def set_logger(args):
    ...
    # Ensure the log directory exists before creating FileHandler.
...
def main(args):
    ...
    # On CPU, loading both summary and entity Pegasus models can exceed RAM.
    # Fall back to summary-only inference/training to avoid OOM.
    if args.device.type == "cpu" and args.model == "PegasusMixPrompt" and args.use_tagger:
        logger.warning("CUDA is unavailable; disabling entity tagger/entity-chain inference to reduce memory usage.")
        args.use_tagger = False
        args.use_entity_chain = False
        args.infer_val_entities = False
            ...
            if args.use_tagger and args.model in ["T5MixPrompt", "PegasusMixPrompt"] and args.guidance_mode != "target":
                if args.infer_val_entities:
                    ...
                    if args.use_pretrain_ckpt:
                        ckpt = torch.load(args.pretrain_ckpt, map_location=args.device)
                        dic = {}
                        for x in ckpt.keys():
                            if (args.max_position_embeddings > 1024) and ("embed_positions" in x):
                                continue
